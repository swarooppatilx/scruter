    ...
    <div className="bg-gray-50 text-gray-800">
      ...
      <section className="relative h-[70vh] flex items-center justify-center bg-blue-300 text-white">
        ...
        <div className="relative z-10 max-w-3xl p-4 text-center">
          <h1 className="text-4xl font-bold mb-4" id="heroTitle">Our Amazing Contributors</h1>
          <p className="text-lg mb-6" id="heroSubtitle">Shaping the future of scruter, one commit at a time</p>
          <a href="#contribute" className="inline-block bg-white text-blue-600 font-bold py-3 px-6 rounded-full transition hover:bg-blue-200">Become a Contributor</a>
        </div>
      </section>

      <section className="py-16 bg-white">
        <h2 className="text-2xl font-bold text-center mb-12">Project Statistics</h2>
        <div id="stats" className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {loading ? (
      ...
      <section className="py-16 bg-gray-100">
        <h2 className="text-2xl font-bold text-center mb-12">Meet Our Contributors</h2>
        <div id="contributors" className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {loading ? (
...
const ContributorCard: React.FC<{ contributor: any }> = ({ contributor }) => (
  <div className="bg-white p-6 rounded-lg shadow-md flex flex-col items-center">
    <a href={contributor.html_url} target="_blank" className="text-center">
      <img src={contributor.avatar_url} alt={contributor.login} className="w-24 h-24 rounded-full border-4 border-blue-500 mb-4" />
      <h3 className="text-xl font-bold">{contributor.login}</h3>
      <div className="bg-blue-100 text-blue-800 font-semibold py-1 px-3 rounded-full text-sm">{contributor.contributions} contributions</div>
    </a>
  </div>
);
