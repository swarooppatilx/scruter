...
const ContributorCardd: React.FC<ContributorCardProps> = ({
  contributor,
  hasNextCard,
  index,
}) => {
  return (
    <div
      id={`contributor-card-${index}`}
      className="relative text-center w-64 mt-10 md:mb-5 md:mt-3 md:mx-5 md:mr-5 md:w-64 flex-shrink-0 order-1 md:order-2"
    >
      <div className="hidden md:block">
        {hasNextCard && (
          <>
            <Image
              src="/contributorsPage/diamond_green.png"
            ...
            />
            <Image
              src="/contributorsPage/diamond_blue.png"
              alt="Glitter decoration"
              width={100}
              height={100}
      ...
      <div className="mt-2">
        <span
          ...
          onClick={() =>
            window.open(
              `https://github.com/swarooppatilx/scruter/commits/main/?author=${contributor.login}`,
              '_blank'
            )
          }
        >
        ...
        <div className="bg-customTeal md:mt-3 dark:bg-gradient-to-r dark:from-[#4caf50] dark:to-[#e9be1e] text-black font-bold py-1 px-4 rounded-full whitespace-nowrap">
          <button onClick={() => window.open(contributor.html_url, '_blank')}>
            {contributor.login}
          </button>
        </div>
      </div>
...
export default ContributorCardd;
