...
const UnansweredQuestionCard: React.FC<UnansweredQuestionCardProps> = ({
  question,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          response.error || 'An error occurred while submitting your answer.'
        );
      }
    } catch (error) {
      toast.error(
      ...
      <CardContent>
        {isAuthenticated ? (
          ...
          <p className="text-red-500 text-sm">
            Please authenticate yourself to answer the question.
          </p>
        )}
      </CardContent>
    </Card>
