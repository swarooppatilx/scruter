import { Car, CreditCard, SendToBack, ShieldQuestion, ShoppingCart, Store, TicketPercent } from "lucide-react";
...
return (
  ...
  <div className="flex items-center  justify-center py-10 px-4 h-full">
    ...
    <div className="h-full flex flex-col gap-4 md:grid md:grid-cols-2 lg:gap-10 lg:grid-cols-3 ">
      {cardData.map((card) => (
        <Card key={card.title} className="min-w-[250px] text-center max-w-[350px] shadow-md bg-gray-200 dark:bg-gray-700 ">
          <CardHeader>
              <div className="flex gap-2 items-center justify-center">
                  {card.logo}
            <CardTitle className="text-xl md:text-2xl ml-4 font-bold" >{card.title}</CardTitle>

              </div>
            <CardDescription className="text-md text-gray-500 dark:text-gray-200">{card.decription}</CardDescription>
          </CardHeader>
        </Card>
      ))}
    </div>
